...
import { ClientReducer, TransactionReducer, initialState } from "./UserReducer";
...
export default ({ user }) => {
  ...
  const [transaction, dispatchTransact] = React.useReducer(
    TransactionReducer,
    initialState
  );
  ...
  return (
    <React.Fragment>
      <PayClientContext.Provider
        value={{ userState, dispatch, transaction, dispatchTransact }}
      >
        <Switch>
          <Route
            path="/pay"
